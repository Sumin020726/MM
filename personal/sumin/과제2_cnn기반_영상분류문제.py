...
x_val, _, y_val,_ = train_test_split(x_test, y_test, test_size=0.6, random_state=1) # test_size 변경가능 - 값을 늘릴 수록 학습속도 빠름

input_shape = x_train.shape[1:]

#동일조건 유지해야 하는 변수(두 모델 모두 동일하게 적용해야 함) - Epoch 변경 금지, Batch는 16 ~ 128 사이에서 변경가능
g_epoch = 70
g_batch = 64
...
from tensorflow.keras.applications import ●▲■ # 알맞은 라이브러리 import
...
# for transfer learning only
transfermodel = ●▲■(weights='imagenet',include_top=False, # 알맞은 라이브러리 import
